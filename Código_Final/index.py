...
try:
    ...
    app.layout = html.Div(children=[
        ...
        html.P("-Juan Esteban: Observando las gráficas se puede ver cómo la Caracas es la que tiene más estaciones con mayor número de accesos (2) que el Autonorte, la calle 26, la NQS y las Américas, teniendo respectivamente 20 estaciones con dos accesos en la calle 26. Esto podría ser debido a que la Caracas es una zona muy transitada al conectarse con muchas más troncales, siendo necesario que por estación se tengan dos zonas de acceso disponibles. Por otro lado, si tomamos como ejemplo la NQS, que por las gráficas podemos saber que es la troncal con mayor número de estaciones con un acceso, podemos indicar que esto puede ser porque la zona no es muy favorable para generar dos accesos por estación, o que simplemente no llega a ser tan transitada como la Caracas. Sin embargo, al tener únicamente un acceso disponible, haría que la entrada a esas estaciones no sea tan cómoda para el usuario, ya que generalmente al ser una estación de una entrada, si quieres ingresar a ella tendrías que dar una gran vuelta para poder usar el puente peatonal o pasar la calle para entrar en ella."),
        ...
        html.P("-Juan Esteban: Observando la gráfica se puede concluir que la gran mayoría de las estaciones de las Caracas poseen componentes WiFi, lo cual puede ser tanto llamativo como positivo para el cliente. Esto se debe a que generalmente sus usuarios son estudiantes o trabajadores que pueden llegar a aprovechar este tipo de servicio para realizar una investigación a último momento, enviar un trabajo, conectarse a reuniones, etc. Esto sin la necesidad de poseer un plan de datos. Aunque una desventaja que puede llegar a suceder es que la calidad de la red no sea buena para tantos usuarios, me parece que con un buen manejo llegaría a ser un buen servicio brindado por TransMilenio. Cabe indicar que, aunque las troncales tengan pocas estaciones con este servicio, no indica que sea malo, ya que aunque sea un servicio positivo, no lo considero uno necesario al 100%."),
        ...
        html.P("-Juan Esteban: Observando los datos sobre la cantidad de biciestaciones por troncal en el sistema de TransMilenio, se puede ver que el Eje Ambiental, la Calle 6, el Autonorte, el Tunal y la Caracas no tienen biciestaciones, mientras que Soacha, Calle 80 y Suba tienen solo una biciestación cada una. La Carrera 7-10 cuenta con dos biciestaciones, la NQS y la Calle 26 tienen tres cada una, y Las Américas tiene cinco. Esto muestra una distribución desigual de las biciestaciones, lo cual presenta ventajas y desventajas. Las ventajas de contar con biciestaciones incluyen la facilidad de combinar el uso de la bicicleta con el transporte público, lo que puede ahorrar tiempo y costos, además de promover un estilo de vida saludable, reducir el tráfico y la contaminación. Sin embargo, la falta de biciestaciones en varias troncales limita la intermodalidad y puede hacer que el sistema de TransMilenio sea menos atractivo para los usuarios que buscan opciones de transporte más sostenibles. Aumentar la cantidad de biciestaciones, especialmente en las troncales sin este servicio, podría mejorar significativamente la experiencia del usuario y contribuir a una ciudad más sostenible."),
        ...
        html.P("-Juan Esteban : Seleccionar la base de datos no fue una tarea tan complicada; el tema fue pensar en qué base de datos usar de tal forma que cumpliera con lo requerido, que fuera accesible, es decir, que pudiéramos tenerla de forma fácil, que fuera fácil de manejar y entender. Sin embargo, ya teniendo la base de datos, al ir desarrollando el MER, la normalización, las consultas y, sobre todo, la carga de datos, pudimos notar que la base de datos no estaba tan bien cargada. Había datos que no permitían que el trabajo fluyera. Esto se debía a que la base de datos tenía ciertos aspectos que la hacían difícil de manejar, como algunas estaciones de SITP mezcladas en una base de datos de estaciones de TransMilenio, o tenía algunas troncales repetidas y no se indicaban de la forma correcta. Por ende, tocaba adaptarse a esos datos para que pudiéramos seguir desarrollando el trabajo de una forma correcta."),
        html.P("Como mencioné anteriormente, al cargar los datos hubo un pequeño conflicto, pero nada que no se pudo arreglar al final. De igual forma, considero que también hubo un poco de confusión al hacer el uso del método COPY, pero repito, no fue algo que se salió de las manos. Ya en el desarrollo del Dashboard y la conexión, no pareció algo muy complicado, pero sí de cuidado, ya que teníamos que tener claro el puerto que estábamos usando, el nombre de la base de datos, qué gráficas usar, qué consulta realizar para hacer un buen análisis, ya que tuvimos que replantear unas cuantas veces si las consultas eran las adecuadas o no, si de verdad eran útiles, hasta que llegamos a las que tenemos en este momento en la página."),
        html.P("Finalmente, mi opinión sobre el proyecto es que fue positiva, permitía que entendieras los procesos de elaboración de tablas, cargar los datos de forma efectiva, realizar una buena consulta, desarrollar tanto una conexión como un Dashboard, y sobre todo el trabajo en equipo."),
